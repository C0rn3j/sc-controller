...
CASES = {
	...
	"gyroabs": {"action": "gyroabs(ABS_X, ABS_Y, ABS_Z)", "sensitivity": (2.0, 3.0, 4.0), "feedback": ("BOTH",)},
	...
	"circular": {"action": "mouse(REL_HWHEEL)", "circular": True, "sensitivity": (2.0,), "feedback": ("BOTH",)},
	"circularabs": {"action": "mouse(REL_HWHEEL)", "circularabs": True, "sensitivity": (2.0,), "feedback": ("BOTH",)},
	"XY": {
		"X": {"action": "axis(ABS_X)"},
		"Y": {"action": "axis(ABS_Y)"},
		"sensitivity": (
	...
	"hold": {"action": "button(Keys.KEY_W)", "hold": {"action": "menu('Default.menu')"}, "feedback": ["LEFT", 32640]},
	"hipfire": {"action": "hipfire( button(KEY_A), button(KEY_B) )", "feedback": ["BOTH"]},
}
