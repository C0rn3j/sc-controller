...
from scc.actions import AxisAction, ButtonAction, GyroAction
...
class TestModifiers:
	...
	def test_deadzone(self):
		...
		# Two parameters
		a = parser.from_json_data({"action": "axis(ABS_X)", "deadzone": {"upper": 300, "lower": 50}})
		...
		a = parser.from_json_data(
			{
				...
				"doubleclick": {"action": "gyro(ABS_RZ, ABS_RX, ABS_Z)", "sensitivity": [7.0, 8.0, 9.0]},
				"action": "axis(ABS_Z)",
				"sensitivity": [
					10.0,
				],
		...
		a = parser.from_json_data(
			{
				"modes": {
					...
					"X": {"action": "gyro(ABS_RZ, ABS_RX, ABS_Z)", "sensitivity": [8.0, 9.0, 10.0]},
				},
				"action": "axis(ABS_Z)",
				"sensitivity": [
					12.0,
	...
	def test_feedback(self):
		...
		# All parameters
		a = parser.from_json_data({"action": "axis(ABS_X)", "feedback": ["RIGHT", 1024, 8, 2048]})
