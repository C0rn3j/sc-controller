#!/usr/bin/env python3
"""sc2.py - reverse-engineering capture harness for the Steam Controller (2026).

Puck (28de:1304), report 0x42 (main gamepad state).
...
def diff(base_label: str, label: str, margin: int = 4) -> None:
	...
	if os.path.exists(imu_path):
		imu = [open(imu_path, "rb").read()[j : j + FRAME_LEN] for j in range(0, os.path.getsize(imu_path), FRAME_LEN)]
		sensor |= noisy(imu, L, 2 * margin)
	print(
		f"diff: baseline={base_label} ({len(base)} fr) vs {label} ({len(ctrl)} fr)"
...
def _walk(items: list[tuple[str, str]], manifest: list[str]) -> None:
	for name, how in items:
		print(f"\n>>> NEXT: {name.upper()}  —  {how}")
		for s in (5, 4, 3, 2, 1):
			sys.stdout.write(f"    starting in {s}...   \r")
			sys.stdout.flush()
			time.sleep(1)
...
GRIPWALK = [
	(
		"notouch_base",
		"Hold the controller ONLY by the TOP edge / trackpads with your\n"
		"      fingertips — palms and fingers OFF both handles. Hold still.",
	),
	(
		"hold_base",
		"Now hold NORMALLY in both hands (palms wrapped around the handles),\n"
...
def report() -> None:
	...
	for lab in labels:
		...
		ctrl, motion = [], []
		for i in range(L):
			cmin = min(f[i] for f in fr)
			cmax = max(f[i] for f in fr)
			chbits = 0
			for f in fr:
				chbits |= f[i] ^ rmode[i]
			if chbits == 0:
